...
def wss(path: Path, format: str) -> dict[str, float]:
    ...
    print(f"File size {str(path)}: {size}")
    if format == "Kia":
    ...
    print(f"Total bytes requested: {bytes_requested}")
    ...
    print(f"Working set size [bytes]: {real_wss}")
    first_wss = sum(first_cache_size.values())
    print(f"Naive WSS (based on first size) [bytes]: {first_wss}")
    last_wss = sum(last_cache_size.values())
    print(f"Naive WSS (based on last size) [bytes]: {last_wss}")
