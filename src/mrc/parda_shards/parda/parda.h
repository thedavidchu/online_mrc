...
void
classical_tree_based_stackdist(char *inputFileName, long lines);
/*functions for glib*/
gboolean
compare_strings(gconstpointer a, gconstpointer b);
void
iterator(gpointer key, gpointer value, gpointer ekt);
...
parda_input_with_binaryfilepointer(FILE *fp,
                                   ...
                                   long end);
void
parda_free(program_data_t *pdt);
end_keytime_t
parda_generate_end(const program_data_t *pdt);
processor_info_t
parda_get_processor_info(int pid, int psize, long sum);
void
parda_get_abfront(program_data_t *pdt_a,
                  const narray_t *gb,
                  const processor_info_t *pit_a);
int
parda_get_abend(program_data_t *pdt_b, const end_keytime_t *ekt_a);
program_data_t
...
void
parda_print_front(const program_data_t *pdt);
void
parda_print_end(const end_keytime_t *ekt);
void
parda_print_tree(const program_data_t *pdt);
void
parda_print_hash(const program_data_t *pdt);
void
parda_print(const program_data_t *pdt);
void
print_iterator(gpointer key, gpointer value, gpointer ekt);
void
parda_print_histogram(const unsigned *histogram);

int
parda_findopt(char *option, char **value, int *argc, char ***argv);
void
parda_process(char *input, T tim, program_data_t *pdt);
/*functions for mpi communication*/
void
show_hkey(void *data, int i, FILE *fp);
void
show_T(void *data, int i, FILE *fp);
/*functions for clock*/
...
static inline T
parda_low(int pid, int psize, T sum)
{
    return (((long long)(pid)) * (sum) / (psize));
}

static inline T
parda_high(int pid, int psize, T sum)
{
    return parda_low(pid + 1, psize, sum) - 1;
}

static inline T
parda_size(int pid, int psize, T sum)
{
    return (parda_low(pid + 1, psize, sum)) - (parda_low(pid, psize, sum));
}

static inline T
parda_owner(T index, int psize, T sum)
{
    return (((long long)psize) * (index + 1) - 1) / sum;
}

static inline char *
parda_generate_pfilename(char filename[], int pid, int psize)
{
    char pfilename[30];
    sprintf(pfilename, "%d_%s_p%d.txt", psize, filename, pid);
    return g_strdup(pfilename);
}
