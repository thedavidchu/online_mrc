...
private:
    ...
    void
    remove_lfu(uint64_t const victim_key,
...
public:
    ...
    {
        // This would make sense if we support multiple buckets of eviction.
        // However, it breaks our plotting code, so that's why I hide it.
        if (false) {
            std::function<std::string(HashList const &val)> lambda =
                [&](HashList const &val) -> std::string {
                RemainingLifetime rl{val, map_, access.timestamp_ms, 100};
                return rl.json();
            };
            return map2str(lfu_cache_, lambda);
        }

        RemainingLifetime rl{lfu_cache_.at(1), map_, access.timestamp_ms, 100};
        return rl.json();
    }
