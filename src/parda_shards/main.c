...
#else
        printf(
            "openmp is not enabled, try to define enable_omp and add OMP variable in Makefile\n");
        abort();
#endif
...
#else
        printf("hybridis not enabled, try to define enable_omp and enable_mpi and add MPI and OMP "
