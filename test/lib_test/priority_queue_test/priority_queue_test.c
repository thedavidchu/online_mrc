...
test_splay_priority_queue(void)
{
    ...
    g_assert_true(SplayPriorityQueue__init(&pq, 10));

    for (Hash64BitType i = 0; i < 10; ++i) {
        // The pq should not be full until we reach the end of this loop!
        g_assert_false(SplayPriorityQueue__is_full(&pq));
        g_assert_true(SplayPriorityQueue__insert_if_room(&pq,
                                                         (Hash64BitType)i,
                                                         (EntryType)i));
    }
    // The pq should now be full!
    g_assert_true(SplayPriorityQueue__is_full(&pq));
    ...
    g_assert_false(SplayPriorityQueue__insert_if_room(&pq,
                                                      (Hash64BitType)10,
                                                      (EntryType)10));

    // Get maximum
    Hash64BitType hash = SplayPriorityQueue__get_max_hash(&pq);
    g_assert_cmpuint(hash, ==, 9);

    // Remove maximum
    EntryType entry = 0;
    g_assert_true(SplayPriorityQueue__remove(&pq, hash, &entry));
    g_assert_cmpuint(entry, ==, 9);
    g_assert_false(SplayPriorityQueue__remove(&pq, hash, &entry));

    // Insert and remove duplicate
    hash = 8;
    entry = 9;
    g_assert_false(SplayPriorityQueue__is_full(&pq));
    g_assert_true(SplayPriorityQueue__insert_if_room(&pq, hash, entry));
    EntryType entry_0, entry_1;
    hash = SplayPriorityQueue__get_max_hash(&pq);
    g_assert_cmpuint(hash, ==, 8);
    g_assert_true(SplayPriorityQueue__remove(&pq, hash, &entry_0));
    hash = SplayPriorityQueue__get_max_hash(&pq);
    g_assert_cmpuint(hash, ==, 8);
    g_assert_true(SplayPriorityQueue__remove(&pq, hash, &entry_1));
    hash = SplayPriorityQueue__get_max_hash(&pq);
    ...
    for (uint64_t i = 0; i < 7; ++i) {
        ...
        // Remove
        g_assert_true(
            SplayPriorityQueue__remove(&pq, expected_max_hash, &entry));
    ...
    SplayPriorityQueue__destroy(&pq);
}
