...
access_same_key_five_times()
{
    ...
    ASSERT_TRUE_OR_CLEANUP(olken_reuse_stack_init(&me, MAX_NUM_UNIQUE_ENTRIES),
                           printf("No cleanup required\n"));

    olken_reuse_stack_access_item(&me, 0);
    olken_reuse_stack_access_item(&me, 0);
    olken_reuse_stack_access_item(&me, 0);
    olken_reuse_stack_access_item(&me, 0);
    olken_reuse_stack_access_item(&me, 0);
    ...
    if (me.histogram.histogram[0] != 4 || me.histogram.false_infinity != 0 ||
        ...
        olken_reuse_stack_print_sparse_histogram(&me);
        assert(0 && "histogram should be {0: 4, inf: 1}");
        olken_reuse_stack_destroy(&me);
        return false;
    }
...
trace_test()
{
    ...
    ASSERT_FUNCTION_RETURNS_TRUE(olken_reuse_stack_init(&me, MAX_NUM_UNIQUE_ENTRIES));

    for (uint64_t i = 0; i < trace_length; ++i) {
        uint64_t key = zipfian_random_next(&zrng);
        olken_reuse_stack_access_item(&me, key);
    }
