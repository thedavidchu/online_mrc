...
public:
    void
    init(uint64_t items, double theta, uint64_t urnd_seed)
    {
        max_ = items - 1;
        theta_ = theta;
        zetan_ = zeta(items);
        ...
        eta_ =
            (1 - std::pow(2.0 / static_cast<double>(items), 1 - theta_)) / (1 - zeta(2) / zetan_);
    ...
    next()
    {
        ...
        uint64_t ret = static_cast<uint64_t>(static_cast<double>(max_) *
