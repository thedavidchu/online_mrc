...
const KeyType random_keys_1[100] = {
    57, 59, 91, 50, 80, 51, 55, 93, 79, 2,  14, 4,  10, 46, 89, 73, 62,
    32, 48, 64, 23, 49, 31, 88, 36, 85, 12, 58, 26, 15, 97, 27, 90, 13,
    52, 1,  30, 53, 68, 42, 92, 71, 40, 66, 77, 17, 86, 35, 60, 63, 20,
    29, 9,  22, 98, 96, 19, 45, 8,  5,  25, 18, 69, 78, 33, 0,  54, 70,
    83, 94, 28, 65, 47, 75, 95, 39, 38, 41, 87, 24, 11, 43, 34, 3,  67,
    16, 37, 61, 81, 99, 84, 82, 72, 6,  74, 7,  44, 76, 21, 56};
const KeyType random_keys_2[100] = {
    92, 35, 40, 99, 37, 32, 11, 28, 36, 84, 47, 50, 86, 73, 77, 64, 90,
    17, 15, 69, 42, 5,  25, 59, 29, 23, 52, 96, 70, 46, 34, 74, 67, 60,
    30, 93, 22, 97, 89, 76, 16, 13, 68, 20, 21, 78, 14, 4,  62, 98, 24,
    63, 51, 85, 80, 49, 41, 75, 66, 55, 91, 53, 10, 33, 81, 82, 79, 44,
    71, 56, 39, 61, 87, 18, 58, 88, 31, 1,  9,  83, 45, 2,  57, 19, 26,
    38, 6,  27, 8,  72, 3,  0,  12, 7,  43, 94, 54, 65, 48, 95};
const KeyType random_keys_3[100] = {
    93, 22, 56, 72, 59, 98, 28, 43, 6,  82, 65, 88, 0,  30, 62, 26, 68,
    7,  36, 9,  90, 47, 52, 75, 50, 49, 13, 2,  71, 92, 44, 35, 95, 10,
    37, 77, 58, 81, 5,  32, 8,  70, 38, 20, 41, 17, 86, 60, 61, 78, 39,
    80, 94, 34, 14, 79, 51, 67, 53, 21, 76, 85, 99, 89, 73, 91, 15, 40,
    69, 45, 83, 16, 27, 48, 74, 87, 96, 18, 42, 84, 57, 54, 19, 25, 55,
    12, 46, 4,  24, 63, 11, 23, 1,  97, 66, 3,  29, 33, 31, 64};
...
{
    struct Tree *tree = tree__new();
    if (tree == NULL) {
        return false;
    }
    for (KeyType i = 0; i < 10; ++i) {
        bool r = tree__insert(tree, i);
        printf("Inserted %" PRIu64 ": %d\n", (uint64_t)i, r);
        tree__print(tree);
    }
    for (KeyType i = 0; i < 11; ++i) {
        bool r = tree__search(tree, i);
        printf("Found %" PRIu64 ": %d\n", (uint64_t)i, r);
    }
    for (KeyType i = 0; i < 11; ++i) {
        bool r = tree__remove(tree, i);
...
random_test_for_naive()
{
    ...
    for (uint64_t i = 0; i < 100; ++i) {
        ...
        bool r = tree__insert(tree, key);
        ASSERT_TRUE_OR_RETURN_FALSE(r, "insert should succeed", tree);
        r = tree__validate(tree);
        ASSERT_TRUE_OR_RETURN_FALSE(
            r,
            "validation following insert should succeed",
            tree);
    }
    for (uint64_t i = 0; i < 100; ++i) {
        KeyType key = random_keys_0[i];
        bool r = tree__search(tree, key);
        ASSERT_TRUE_OR_RETURN_FALSE(r, "search should succeed", tree);
        uint64_t reverse_rank = tree__reverse_rank(tree, key);
        if (reverse_rank != 99 - key) {
            ...
            tree__free(tree);
            assert(0 &&
                   "reverse rank should be 99 - key for keys in range 0..=99");
            return false;
        }
        r = tree__validate(tree);
        ASSERT_TRUE_OR_RETURN_FALSE(
            r,
            "validation following search should succeed",
            tree);
    }
    ...
    for (uint64_t i = 0; i < 100; ++i) {
        KeyType key = random_keys_0[i];
        bool r = tree__remove(tree, key);
        ASSERT_TRUE_OR_RETURN_FALSE(r, "remove should succeed", tree);
        r = tree__validate(tree);
        ASSERT_TRUE_OR_RETURN_FALSE(
            r,
            "validation following remove should succeed",
            tree);
    }
    tree__free(tree);
    return true;
...
random_test_with_different_traces_for_sleator()
{
    ...
    for (uint64_t i = 0; i < 100; ++i) {
        KeyType key = random_keys_3[i];
        bool r = tree__sleator_remove(tree, key);
        ASSERT_TRUE_OR_RETURN_FALSE(r, "remove should succeed", tree);
        r = tree__validate(tree);
        ASSERT_TRUE_OR_RETURN_FALSE(
            r,
