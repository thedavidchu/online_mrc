...
test_splay_priority_queue(void)
{
    ...
    g_assert_true(splay_priority_queue_init(&pq, 10));

    for (Hash64BitType i = 0; i < 10; ++i) {
        // The pq should not be full until we reach the end of this loop!
        g_assert_false(splay_priority_queue_is_full(&pq));
        g_assert_true(splay_priority_queue_insert_if_room(&pq, (Hash64BitType)i, (EntryType)i));
    }
    // The pq should now be full!
    g_assert_true(splay_priority_queue_is_full(&pq));
    ...
    g_assert_false(splay_priority_queue_insert_if_room(&pq, (Hash64BitType)10, (EntryType)10));

    // Get maximum
    Hash64BitType hash = splay_priority_queue_get_max_hash(&pq);
    g_assert_cmpuint(hash, ==, 9);

    // Remove maximum
    EntryType entry = 0;
    g_assert_true(splay_priority_queue_remove(&pq, hash, &entry));
    g_assert_cmpuint(entry, ==, 9);
    g_assert_false(splay_priority_queue_remove(&pq, hash, &entry));

    // Insert and remove duplicate
    hash = 8;
    entry = 9;
    g_assert_false(splay_priority_queue_is_full(&pq));
    g_assert_true(splay_priority_queue_insert_if_room(&pq, hash, entry));
    EntryType entry_0, entry_1;
    hash = splay_priority_queue_get_max_hash(&pq);
    g_assert_cmpuint(hash, ==, 8);
    g_assert_true(splay_priority_queue_remove(&pq, hash, &entry_0));
    hash = splay_priority_queue_get_max_hash(&pq);
    g_assert_cmpuint(hash, ==, 8);
    g_assert_true(splay_priority_queue_remove(&pq, hash, &entry_1));
    hash = splay_priority_queue_get_max_hash(&pq);
    ...
    for (uint64_t i = 0; i < 7; ++i) {
        ...
        // Remove
        g_assert_true(splay_priority_queue_remove(&pq, expected_max_hash, &entry));
        ...
        g_assert_false(splay_priority_queue_remove(&pq, expected_max_hash, &entry));
    }

    splay_priority_queue_destroy(&pq);
}
...
main(int argc, char *argv[])
{
    ...
    g_test_add_func("/priority_queue_test/splay_priority_queue_test", test_splay_priority_queue);
