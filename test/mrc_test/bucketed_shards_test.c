...
access_same_key_five_times(void)
{
    ...
    g_assert_true(BucketedShards__init(&me, MAX_NUM_UNIQUE_ENTRIES, 1));
...
small_exact_trace_test(void)
{
    ...
    g_assert_true(BucketedShards__init(&me, 1024, 10));
...
long_accuracy_trace_test(void)
{
    ...
    // The maximum trace length is obviously the number of possible unique items
    g_assert_true(Olken__init(&oracle, MAX_NUM_UNIQUE_ENTRIES));
    g_assert_true(BucketedShards__init(&me, 1 << 12, MAX_NUM_UNIQUE_ENTRIES));
