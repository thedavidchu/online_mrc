...
const size_t random_values_0_to_11[100] = {
...
#define ASSERT_TRUE_OR_RETURN_FALSE(r, msg, histogram_ptr)                                         \
    do {                                                                                           \
        if (!(r)) {                                                                                \
            /* Clean up resources */                                                               \
            basic_histogram_destroy((histogram_ptr));                                              \
...
test_basic_histogram()
{
    ...
    bool r = basic_histogram_init(&hist, 10);
    if (!r) {
        return false;
    }

    for (size_t i = 0; i < 100; ++i) {
        r = basic_histogram_insert_finite(&hist, random_values_0_to_11[i]);
        if (!r) {
            return false;
        }
    }
    for (size_t i = 0; i < 3; ++i) {
        r = basic_histogram_insert_infinite(&hist);
        if (!r) {
            return false;
        }
    }
    ...
    const size_t histogram_oracle[] = {9, 9, 12, 9, 4, 8, 15, 9, 6, 8};
    const size_t length_oracle = 10;
    const size_t false_infinity_oracle = 11;
    const size_t infinity_oracle = 3;
    const size_t running_sum_oracle = 103;
